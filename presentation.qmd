...
elements <- data.frame(
  ...
  element = c("Data", "Aesthetics", "Layers", "Scales", "Facets", 
...
ggplot() +
  geom_shape(
    data = shapes, 
    aes(x = x, y = y2, fill = factor(id)),
    alpha = 0.6) +
  coord_cartesian(xlim = c(0, 2.5), ylim = c(0, number_of_shapes + 1)) +
  geom_text(
    data = elements,
    aes(x = 1.6, y = y, label = element, color = factor(id)),
    hjust = 0, vjust = -0.2, size = 6, family = "Poppins SemiBold"
    ) +
  scale_fill_manual(values = MetBrewer::met.brewer("Moreau"),
